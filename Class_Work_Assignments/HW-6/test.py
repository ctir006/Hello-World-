def best_help(i,w,w_i,v_i,c_i,d):
	if (i,w,c_i[i]) in d:
		return d[(i,w,c_i[i])][0]
	if i==-1 or w==0:
		return 0
	elif c_i[i]==0:
		d[(i,w,c_i[i])]=(best_help(i-1,w,w_i,v_i,c_i,d),0,(i-1,w,c_i[i-1]))
		return d[(i,w,c_i[i])][0]
	elif w_i[i]>w:
		d[(i,w,c_i[i])]=(best_help(i-1,w,w_i,v_i,c_i,d),0,(i-1,w,c_i[i-1]))
		return d[(i,w,c_i[i])][0]
		...
		k=v_i[i]
		c_i[i]-=1
		l=k+best_help(i,w-w_i[i],w_i,v_i,c_i,d)
		k_i=c_i
		c_i[i]+=1
		m=best_help(i-1,w,w_i,v_i,c_i,d)
		if l>m:
...
def best(w,l):
	...
	for i in l:
		...
		c_i.append(i[2])
	d={}
	rr=best_help(len(l)-1,w,w_i,v_i,c_i,d)
	# for i in d:
		# print(i,d[i])
	key=list(d.keys())[-1]
	#print(key)
